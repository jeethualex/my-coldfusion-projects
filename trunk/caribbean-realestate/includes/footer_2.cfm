<cfoutput>
  ...
  <div class="content tabcontent checkerLightBG"> <a name="featured_properties"></a>
    <table border="0" cellpadding="2" cellspacing="0" class="categories">
      <tr>
        <cfset myQuery2 = obj.ListFCategories()/>
        <cfloop query="myQuery2">
          <td width="100" <cfif currentrow gt 1>class="dottedLineLt"<cfelse><cfset FirstCategoryId = category_id></cfif> align="center"><a href="##featured_properties" onclick="javascript:$('.loadArea').load('#application.incPath#ajax_featured_category.cfm?catid=#category_id#&items=3');">#obj.GetCategory(category_id).name#</a></td>
        </cfloop>
