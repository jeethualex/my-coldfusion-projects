...
<cfif IsDefined("form.Submit")>
  ...
  <cfif keyword neq "">
    ...
    <cfset myQuery = createObject("component", "#application.commonCFC#").SearchState(form)/>
    <cfelse>
    <cfset StructDelete(Session,"keyword")>
    <cfset StructDelete(Session,"type")>
    <cfset myQuery = createObject("component", "#application.commonCFC#").ListStates()/>
  </cfif>
  ...
  <cfset type = frm.type>
  <cfset myQuery = createObject("component", "#application.commonCFC#").SearchState(frm)/>
  <cfelse>
  <cfset myQuery = createObject("component", "#application.commonCFC#").ListStates()/>
</cfif>
...
<cfoutput>
  ...
  <table class="noborder">
    <tr>
      ...
      <td>|
        <input name="newState" type="button" value="Add New State" onclick="javascript:$(location).attr('href','#application.incPath#state_add.cfm');" /></td>
    </tr>
  </table>
</cfoutput>
    ...
    <cfoutput query="myQuery" startrow="#pagination.getStartRow()#" maxrows="#pagination.getMaxRows()#">
      <tr>
        ...
        <td><cfset qryStr = "id=#id#&page=#page#">
          <a href="state_edit.cfm?#qryStr#">edit</a> | <a href="state_delete.cfm?#qryStr#" onclick="javascript:return confirm('You are about delete this record. Are you sure.')">delete</a></td>
      </tr>
    </cfoutput>
  </table>
