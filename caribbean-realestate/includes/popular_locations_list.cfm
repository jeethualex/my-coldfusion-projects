...
<cfoutput>
  <h1>Popular Locations List</h1>
  <table class="noborder">
    <tr>
      <td><input name="newcategory" type="button" value="Add New Popular Location" onclick="javascript:$(location).attr('href','#application.incPath#popular_locations_add.cfm');" /></td>
    </tr>
  </table>
</cfoutput>
<cfif myQuery.recordCount>
  <table class="gridd">
    ...
    <cfoutput query="myQuery" startrow="#pagination.getStartRow()#" maxrows="#pagination.getMaxRows()#">
      <tr>
        <td>#obj.GetLocation(location_id).name#</td>
        <td>#ordernum#</td>
        <td><cfset qryStr = "id=#id#&page=#page#">
          <a href="popular_locations_edit.cfm?#qryStr#">edit</a> | <a href="popular_location_delete.cfm?#qryStr#" onclick="javascript:return confirm('You are about delete this record. Are you sure.')">delete</a></td>
      </tr>
    </cfoutput>
  </table>
